...
//				}
//				
////				if (worldHeight[x][y] % 5 == 0) {
////					int index = y * world.getY() + x;
////					super.colorDiffuseList.remove(index);
////					super.colorAmbientList.remove(index);
////					super.colorDiffuseList.add(index, new float[]{0f, 0f, 1f});
////					super.colorAmbientList.add(index, new float[]{0f, 0f, 1f});
////				}
//				i++;
//			}
//		}
...
//	}
	...
	public MapMesh(World world) {
		...
		listFill(verticesList, new float[]{-1, -1, -1}, world.getY() * amountPerRow + world.getX() + 1);
		listFill(colorAmbientList, new float[]{1, 1, 1}, world.getY() * amountPerRow + world.getX() + 1);
		listFill(colorDiffuseList, new float[]{1, 1, 1}, world.getY() * amountPerRow + world.getX() + 1);

		for (int y = 0; y < world.getY(); y++) {

			for (int x = 0; x < world.getX(); x++) {
				
				float tempHeight = worldHeight[x][y] + random.nextFloat() / 4 -0.125f;
//				float tempHeight = worldHeight[x][y];
				...
				centerHeight = (bottomLeftHeight + bottomRightHeight + upperLeftHeight + upperRightHeight) / 4;
				
				if (0 < x && x < world.getX() - 1 && 0 < y && y < world.getY() - 1) {
					
					if (world.isBlocked(x, y)) {
						colorDiffuseList.set(centerIndex, new float[] { 1f, 0f, 0f });
						colorAmbientList.set(centerIndex, new float[] { 1f, 0f, 0f });
					} else {
						colorDiffuseList.set(centerIndex, new float[] { 0f, 1f, 0f });
						colorAmbientList.set(centerIndex, new float[] { 0f, 1f, 0f });
					}
				} else {
				...
				verticesList.set(bottomLeftIndex, new float[] {x - MID, y - MID, bottomLeftHeight});
				verticesList.set(bottomRightIndex, new float[] {x + MID, y - MID, bottomRightHeight});
				verticesList.set(upperLeftIndex, new float[] {x - MID, y + MID, upperLeftHeight});
				verticesList.set(upperRightIndex, new float[] {x + MID, y + MID, upperRightHeight});
				verticesList.set(centerIndex, new float[] {x, y, centerHeight});
				
				indicesList.add(new int[]{bottomLeftIndex, bottomRightIndex, centerIndex});
				indicesList.add(new int[]{bottomRightIndex, upperRightIndex, centerIndex});
				indicesList.add(new int[]{upperRightIndex, upperLeftIndex, centerIndex});
				indicesList.add(new int[]{upperLeftIndex, bottomLeftIndex, centerIndex});
			}
		}
